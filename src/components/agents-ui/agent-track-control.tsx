...
export function AgentTrackControl({
  ...
  const shouldRequestPermissions = requestPermissionsState || Boolean(pressed && kind);
  ...
  const handlePressedChange = (nextPressed: boolean) => {
    // Once the user interacts with this control, keep the enumerated device
    // list warm so its selector does not mount and unmount between toggles.
    setRequestPermissionsState(true);
